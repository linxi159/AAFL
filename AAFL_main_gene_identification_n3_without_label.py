...
if __name__ == '__main__':
    ...
    if args.train == True:
        ...
        #batch_size_ = 1
        #reconstruction = test2(test_data, model, criterion, batch_size_,labels_predict)
        #reconstruction = test3(test_data, model, criterion, batch_size_,labels_predict, Left_all)
        #now = datetime.datetime.now()
        
        print('fea_scaler: ', fea_scaler[0].detach().cpu().numpy())
        print('fea_scaler_sum: ', np.sum(fea_scaler[0].detach().cpu().numpy()))
        print('fea_scaler[0].shape: ', fea_scaler[0].shape)
        print('Left_all: ', Left_all[0].detach().cpu().numpy())
        print('Left_all_sum: ', np.sum(Left_all[0].detach().cpu().numpy()))
        print('Left_all[0].shape: ', Left_all[0].shape)
        print('Left_all: ', Left_all)        
 
        fea_weight = fea_scaler[0].detach().cpu().numpy()
        np.savetxt('./result_n3/feature_weight_n3_without_label.csv', fea_weight, delimiter = ',')
        np.savetxt('./result_n3/labels_predict_n3_without_label.csv', labels_predict_, delimiter = ',')
       
        #print(reconstruction)
        #print('len(reconstruction): ', len(reconstruction))  
        #print('reconstruction[0].shape: ', reconstruction[0].shape) 
